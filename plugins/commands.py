...
from info import PM_LOG_CHANNEL, LOG_CHANNEL2, TUTORIAL_LINK, MALIK, MALIK5, CHANNELS, ADMINS, AUTH_CHANNEL, PHTT, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT
...
async def start(client:Client, message):
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        await db.update_notcopy_user(user_id, {"last_verified":datetime.now()})
...
#New_Verified_User_complete"""
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔸 sᴜʙsᴄʀɪʙᴇ ᴍʏ ʏᴛ ᴄʜᴀɴɴᴇʟ 🔸",url="https://youtube.com/channel/UCPaHDqWf3D3w2nxb8p3sr4A"),]]),parse_mode=enums.ParseMode.HTML)#"You are now verified for next 24 hours. Continue asking movies")
...
#pm verify 
    mf = message
    user_id = mf.from_user.id
    
    if len(mf.command) == 2 and mf.command[1].startswith('nottcopy'):
        user_id = mf.command[1].split("_")[1]
        await db.update_nottcopy_user(user_id, {"last_verified":datetime.now()})
        text = f"""User ID : `{user_id}`
Username : {mf.from_user.mention}
Time : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

#New_Verified_User_complete"""
        await client.send_message(LOG_CHANNEL2, text)
        dmm = await mf.reply_photo(
        photo=(MALIK5), 
        caption=(MALIK7.format(message.from_user.mention)), 
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔸 sᴜʙsᴄʀɪʙᴇ ᴍʏ ʏᴛ ᴄʜᴀɴɴᴇʟ 🔸",url="https://youtube.com/channel/UCPaHDqWf3D3w2nxb8p3sr4A"),]]),parse_mode=enums.ParseMode.HTML)#"You are now verified for next 24 hours. Continue asking movies")
        return #await client.send_message(LOG_CHANNEL, text)
...
#New_Verify_User"""
    ...
    await client.send_cached_media(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton('♻️ 𝙂𝙧𝙤𝙪𝙥', url="https://t.me/+FAgX05kGByNkZjJl"),
        ...
        buttons = [
            ...
            [
                ...
                InlineKeyboardButton(
                    ...
                    callback_data=f'setgs#botpm#{settings["bottpm"]}#{grp_id}',
                ),
            ],
            [
                InlineKeyboardButton(
